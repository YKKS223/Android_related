...
class ExtendedKalmanFilter {
    ...
    private val SHORT_DISTANCE_THRESHOLD = 2.0f //m(これ未満の移動ではGPS更新を抑制)
    ...
    private val GPS_XY_NOISE_FACTOR = 0.5f //例: GPS精度を半分として扱う (要調整！)
    ...
    private val GPS_MIN_STD_DEV = 2.0f

    /**
     * EKF 更新ステップ (GPS位置観測)
     * @param gpsData GPSマネージャーから受け取ったデータ
     */
    fun updateGpsPosition(gpsData: LocationData) {
        //短距離移動チェック
        if (lastEkfPositionForShortDistanceCheck != null) {
            val currentEkfPos = getPosition() //現在のEKF位置
            val dxEkf = currentEkfPos[0] - lastEkfPositionForShortDistanceCheck!![0]
            val dyEkf = currentEkfPos[1] - lastEkfPositionForShortDistanceCheck!![1]
            //Zも含める場合: val dzEkf = currentEkfPos[2] - lastEkfPositionForShortDistanceCheck!![2]
            val ekfDistanceMoved = sqrt(dxEkf * dxEkf + dyEkf * dyEkf /* + dzEkf * dzEkf */) //XY平面で計算

            if (ekfDistanceMoved < SHORT_DISTANCE_THRESHOLD) {
                Log.d("EKF_Update_GPS", "Skipping GPS update: Short distance moved since last GPS update (%.2fm < %.1fm)".format(ekfDistanceMoved, SHORT_DISTANCE_THRESHOLD))
                //短距離移動なので、GPS更新をスキップする
                //この場合、最後にGPS更新した位置の情報 (lastEkfPositionForShortDistanceCheck) は更新しない
                return
            } else {
                Log.d("EKF_Update_GPS", "Proceeding with GPS update: Distance moved %.2fm >= %.1fm".format(ekfDistanceMoved, SHORT_DISTANCE_THRESHOLD))
                //閾値を超えたので、この後GPS更新を実行し、lastEkfPositionForShortDistanceCheck も更新する
            }
        } else {
            //初回のGPS更新、またはリセット後の最初の更新
            Log.d("EKF_Update_GPS", "Proceeding with first GPS update after reset or initialization.")
            //この後GPS更新を実行し、lastEkfPositionForShortDistanceCheck を設定する
        }

        //既存のGPS有効性チェック
        val maxAccuracy = 50.0f
        if (!gpsData.isLocalValid || gpsData.accuracy == null || gpsData.accuracy > maxAccuracy) {
            //Log.w("EKF_Update_GPS", "Skipping GPS update: Invalid or too inaccurate...")
            return
        }
        if (gpsData.localX == null || gpsData.localY == null) {
            //Log.w("EKF_Update_GPS", "Skipping GPS update: Null local XY coordinates.")
            return
        }
        ...
        if (useZ) {
            ...
            val stdDevXY = gpsAccuracyStdDev * GPS_XY_NOISE_FACTOR
            val stdDevZ = gpsAccuracyStdDev * GPS_Z_NOISE_FACTOR
        ...
        lastEkfPositionForShortDistanceCheck = getPosition().clone()
    }
    ...
    fun reset() {
        ...
        //リセット時に履歴もクリア
        lastEkfPositionForShortDistanceCheck = null
