...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private val GPS_CONSISTENCY_DISTANCE_THRESHOLD = 5.0f //m(EKFがこれ以上動いたのにGPSが動かない場合に補正)
    ...
    private fun processLinearAcceleration(event: SensorEvent) {
        ...
        val remainsStationary = isStationary && (accelMagnitude < stationaryAccelThreshold * 1.5f && gyroMagnitude < stationaryGyroThreshold * 1.5f) // ヒステリシス
        ...
        if (gpsDataToUse != null && gpsDataToUse.isLocalValid) {
            if (lastValidGpsData == null || gpsDataToUse.timestamp > lastValidGpsData!!.timestamp) {
                ...
                ekf.updateGpsPosition(gpsDataToUse)
                gpsUpdated = true
    ...
    private fun updateUI() {
        ...
        runOnUiThread {
            ...
            if (isStationary) {
                ...
                // 低速度クリッピングが有効な状態を示すことも可能
                if (worldSpeed < MIN_VELOCITY_THRESHOLD) {
